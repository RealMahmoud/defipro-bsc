  ...
  <div>
        ...
        <div class="card-header border-0">
          ...
          <div class="col text-right">
            <base-button
              ...
              >Track New OTC</base-button
            >
            <base-button type="primary" size="sm" @click="goToTrading"
              >Buy</base-button
            >
            <base-button type="primary" size="sm" @click="goToTrading"
              >Sell</base-button
            ...
            >
              <card class="card-stats" :show-footer-line="true">
                <div class="row">
                  ...
                  <div class="col-auto">
                    <div class="icon icon-shape">
                      <a @click="prepareUnRegister(market)"
                        ><span class="text-danger mr-2"
                          ><i class="fa fa-trash"></i> </span
                      ></a>
                    </div>
                    <div class="icon icon-shape">
                      <a @click="initCloseMarket(market)"
                        ><span class="text-danger mr-2"
                          ><i class="fa fa-store"></i> </span
                      ></a>
                    </div>
                  </div>
                </div>
                <p class="mt-3 mb-0 text-sm">
                  ...
                  >{{ timestampToDate(market.closeTime) }}
                </p>
              </card>
            </div>
          </div>
        ...
        <template>
          ...
          <form role="form">
            ...
            <div class="text-center mt-2">
              <base-button @click="trackNewOtc" type="primary"
                >Track</base-button
              >
            </div>
          </form>
    ...
    >
      ...
      <div class="py-3 text-center">
        ...
        <p>
          This action will not affect the OTC Market contract. It will only
          remove it form your personal list of tracked markets.
        </p>
      </div>
    ...
    <modal
      :show.sync="modals.modalCloseMarket"
      gradient="warning"
      modal-classes="modal-warning modal-dialog-centered"
    >
      ...
      <div class="py-3 text-center">
        ...
        <h4 class="heading mt-4">Information</h4>
        <p>
          This action will close the OTC Market, no further trading will be
          possible after this action.
        </p>
      </div>

      <template slot="footer">
        <base-button type="white" @click="confirmCloseMarket"
          >Ok, close it</base-button
        >
        <base-button
          type="link"
          text-color="white"
  ...
  methods: {
    ...
    deploy(evt) {
      try {
        ...
        const sender = window.ethereum.selectedAddress;
        const closeTimeTimeStamp = moment(closeTime, "YYYY-MM-DD HH:m:s")
          .toDate()
          .getTime();
        this.smartContractManager.deployContract(
          this.matchingMarketContract,
          sender,
          this.matchingMarketArtifacts.bytecode,
    ...
    async deployReceiptCallback(receipt) {
      ...
      const closeTimeTimeStamp = moment(
    ...
    async unTrackMarket(market) {
      unRegisterOtcMarket(this.data.otcMarketStore, market.name);
      await this.initData();
    }
  },
  computed: {
